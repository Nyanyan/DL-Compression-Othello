...
int main(int argc, char* argv[]){
    std::ofstream fout;
    fout.open("eval_8bit_as_16bit.egev", std::ios::out|std::ios::binary|std::ios::trunc);
    if (!fout){
        std::cerr << "can't open eval.egev" << std::endl;
        return 1;
    }
    short elem;
    short max_elem = -127, min_elem = 127;
    std::ifstream ifs("txt_eval_8bit_joined.txt");
    if (ifs.fail()){
